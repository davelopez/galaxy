...
class Registry:
    ...
    def _register_crypt4gh_datatypes(self) -> None:
        ...
        for extension, datatype in existing_datatypes:
            if extension == crypt4gh.CRYPT4GH_FILE_EXT or crypt4gh.is_crypt4gh_file_ext(extension):
                continue
            wrapped_extension = crypt4gh.wrap_crypt4gh_file_ext(extension)
            if wrapped_extension in self.datatypes_by_extension:
                continue
            wrapped_datatype = crypt4gh.build_crypt4gh_datatype(datatype, transparent_staging_enabled)
        ...
        if crypt4gh.CRYPT4GH_FILE_EXT not in self.datatypes_by_extension:
            generic_crypt4gh_datatype = crypt4gh.Crypt4GH()
            generic_crypt4gh_datatype.transparent_staging_enabled = transparent_staging_enabled
            self.datatypes_by_extension[crypt4gh.CRYPT4GH_FILE_EXT] = generic_crypt4gh_datatype
            self.datatypes_by_suffix_inferences[crypt4gh.CRYPT4GH_FILE_EXT] = generic_crypt4gh_datatype
            self.mimetypes_by_extension[crypt4gh.CRYPT4GH_FILE_EXT] = generic_crypt4gh_datatype.get_mime()
            self.log.debug("Dynamically registered crypt4gh datatype: %s", crypt4gh.CRYPT4GH_FILE_EXT)
    ...
    def get_datatype_from_filename(self, name):
        ...
        if name.endswith(crypt4gh.CRYPT4GH_SUFFIX):
            ...
            wrapped_extension = crypt4gh.wrap_crypt4gh_file_ext(inner_datatype.file_ext)
            return self.datatypes_by_extension.get(
                wrapped_extension,
                self.get_datatype_by_extension(crypt4gh.CRYPT4GH_FILE_EXT) or generic_datatype_instance,
            )
