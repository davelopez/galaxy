...
class QuotaManager:
    ...
    def create_quota(self, payload: dict, decode_id=None) -> Tuple[model.Quota, str]:
        ...
        if self.sa_session.query(model.Quota).filter(model.Quota.table.c.name == params.name).first():
    ...
    def rename_quota(self, quota, params) -> str:
        ...
        elif params.name != quota.name and self.sa_session.query(model.Quota).filter(model.Quota.table.c.name == params.name).first():
...
class QuotasManager:
    ...
    def index(self, trans: ProvidesUserContext, deleted: bool = False) -> QuotaSummaryList:
        ...
        if deleted:
            route = 'deleted_quota'
            query = query.filter(model.Quota.table.c.deleted == true())
        else:
            route = 'quota'
            query = query.filter(model.Quota.table.c.deleted == false())
        for quota in query:
    ...
    def validate_in_users_and_groups(self, trans, payload):
        ...
        for item in util.listify(payload.get('in_users', [])):
            try:
                new_in_users.append(get_id(item, model.User, model.User.table.c.email))
            except Exception:
                invalid.append(item)
        for item in util.listify(payload.get('in_groups', [])):
            try:
                new_in_groups.append(get_id(item, model.Group, model.Group.table.c.name))
            except Exception:
                invalid.append(item)
        if invalid:
